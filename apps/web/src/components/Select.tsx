...
export const Select = forwardRef<HTMLSelectElement, Props>(function Select(
  ...
  return (
    <label className="block space-y-1.5" htmlFor={selectId}>
      <span className="text-[12px] font-semibold tracking-tight text-zinc-200">{label}</span>
      <select
        ref={ref}
        id={selectId}
        className={cn(
          'field-base appearance-none bg-[length:1rem] bg-[right_0.75rem_center] bg-no-repeat pr-10',
          error
